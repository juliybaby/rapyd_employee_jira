...
$res= $db->prepare("SELECT count(*) as totalcount FROM salary_emp_rapyd where creator_id=:creator_id  order by id");
$res->execute(array(':creator_id' =>$owner_id));
$t_row = $res->fetch();
$totalcount = $t_row['totalcount'];

if($totalcount == 0){
//echo "<div style='background:red;color:white;padding:10px;border:none;'>No Employees Has been Paid Salary Yet.. <b></b></div>";
echo 11;
exit();
}

$result = $db->prepare("SELECT * FROM salary_emp_rapyd where creator_id=:creator_id  order by id DESC limit :row1, :rowpage");
